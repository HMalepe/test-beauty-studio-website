...
function AppleIcon() {
  return (
    <svg width="22" height="26" viewBox="0 0 22 26" fill="currentColor" aria-hidden className="shrink-0">
...
function GooglePlayIcon() {
  return (
    <svg width="22" height="24" viewBox="0 0 22 24" fill="none" aria-hidden className="shrink-0">
      <path d="M1.1 1.3c-.4.4-.6 1-.6 1.7v17.9c0 .7.2 1.3.6 1.7l.1.1 10-10v-.3L1.2 1.2l-.1.1z" fill="#00C3FF" />
      <path d="M14.5 14.8 11.2 17.6l-10 5.8c.8.8 1.9 1.2 3 1.1l10.3-6-0.1-.1z" fill="#FFBA00" />
      <path d="M14.5 9.2 11.2 6.4 1.2.6C2 .4 3.1.8 3.9 1.6l10.6 7.6z" fill="#FF3A44" />
      <path d="M14.5 12 21.3 8.1c.7-.4 1.1-1 1.1-1.7s-.4-1.3-1.1-1.7l-6.8-3.9-3.3 2.8 3.3 2.8z" fill="#00F076" />
    </svg>
  )
}

function StoreButton({ href, icon, lineOne, lineTwo }: { href: string; icon: ReactNode; lineOne: string; lineTwo: string }) {
  return (
    <a href={href} className="inline-flex items-center gap-3 rounded-full bg-[#111111] px-5 py-3 text-paper transition-colors hover:bg-[#1a1a1a]">
      {icon}
      <span className="flex flex-col items-start leading-tight">
        <span className="text-[10px] font-medium uppercase tracking-wide opacity-80">{lineOne}</span>
...
export function Hero() {
  ...
  useLayoutEffect(() => {
    ...
    if (!section || !words.length || !sub || !btns || !phone) return

    if (reducedMotion) {
      gsap.set([words, sub, btns, phone], { opacity: 1, y: 0 })
      return
    }

    const mobile = prefersNativeScroll()

    gsap.set([words, sub, btns, phone], { opacity: 0, y: mobile ? 18 : 30 })

    const ctx = gsap.context(() => {
      const tl = gsap.timeline({ defaults: { ease: 'power3.out' }, delay: 0.2 })
      tl.to(words, { opacity: 1, y: 0, duration: 0.55, stagger: 0.06 })
        .to(sub, { opacity: 1, y: 0, duration: 0.45 }, '-=0.25')
        .to(btns, { opacity: 1, y: 0, duration: 0.45 }, '-=0.2')
        .to(phone, { opacity: 1, y: 0, duration: 0.55 }, '-=0.15')
    }, section)
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen bg-paper pt-[120px] sm:pt-[140px]">
      <div className="mx-auto flex max-w-[min(96vw,1400px)] flex-col items-center px-6 text-center">
        <h1 className="sr-only">{HEADLINE}</h1>
        <p aria-hidden className="text-hero mx-auto max-w-[min(96vw,1200px)] text-balance text-ink">
        ...
        <div ref={subheadingRef} className="mt-8 max-w-xl text-lg font-normal text-neutral-800 sm:text-xl">
        ...
        <div ref={buttonsRef} className="mt-10 flex flex-wrap items-center justify-center gap-4">
          <StoreButton href="#" icon={<AppleIcon />} lineOne="Download on the" lineTwo="App Store" />
          <StoreButton href="#" icon={<GooglePlayIcon />} lineOne="Get it on" lineTwo="Google Play" />
        </div>
      </div>
      <div ref={phoneRef} className="relative mt-12 w-full sm:mt-20">
        <div className="relative min-h-[380px] w-full overflow-hidden sm:min-h-[520px]">
          <img src={DESK_BG} alt="" className="absolute inset-0 h-full w-full object-cover" loading="eager" decoding="async" />
