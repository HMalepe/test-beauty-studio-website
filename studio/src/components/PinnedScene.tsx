    ...
    <div className="flex h-full flex-col bg-paper pt-10 text-ink">
      ...
      <div className="flex-1 space-y-4 px-4 pt-4">
        <label className="block">
          <span className="mb-2 block text-xs font-medium text-ink/50">Prompt</span>
        ...
        <div>
          <p className="mb-2 text-xs font-medium text-ink/50">Inspiration Photos</p>
          <div className="flex gap-2">
            ...
            <div className="h-16 w-16 overflow-hidden rounded-xl bg-offwhite">
              <img src={PROMPT_INSPIRATION} alt="" className="h-full w-full object-cover" />
            </div>
  ...
  return (
    <div className="relative h-full bg-ink text-paper">
      ...
      <div className="absolute inset-x-4 top-24 overflow-hidden rounded-2xl border border-paper/10 bg-paper/5">
        <img src={PINNED.result1} alt="" className="aspect-square w-full object-cover" />
      </div>
...
export function PinnedScene() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(phone, { scale: 0.88, yPercent: 6, force3D: true })
      gsap.set([s2, s3, s4], { autoAlpha: 0 })
      gsap.set(s1, { autoAlpha: 1 })
      gsap.set(canvas, { autoAlpha: 0, scale: 0.96 })
      gsap.set(sketch, { autoAlpha: 0, scale: 0.92 })
      gsap.set(thumbs, { autoAlpha: 0, scale: 0.7 })
      ...
      tl.to(phone, { scale: 1.05, yPercent: 0, duration: 1 }, 0)

      tl.to(s1, { autoAlpha: 0, duration: 0.4 }, 1)
        .to(s2, { autoAlpha: 1, duration: 0.4 }, 1.1)

      tl.to(s2, { autoAlpha: 0, duration: 0.4 }, 2)
        .to(s3, { autoAlpha: 1, duration: 0.4 }, 2.1)

      tl.to(phone, { scale: 1.4, duration: 0.8 }, 3)
        .to([s1, s2, s3], { autoAlpha: 0, duration: 0.2 }, 3)
        .to(s4, { autoAlpha: 1, duration: 0.15 }, 3)
        .to(phone, { autoAlpha: 0, duration: 0.35 }, 3.3)
        .to(canvas, { autoAlpha: 1, scale: 1, duration: 0.5 }, 3.2)
        .to(sketch, { autoAlpha: 1, scale: 1, duration: 0.4 }, 3.5)
        .to(thumbs, { autoAlpha: 1, scale: 1, duration: 0.4, stagger: 0.12 }, 3.6)
  ...
  if (reducedMotion) {
    return (
      <section ref={root} className="relative min-h-screen w-full" aria-label="AI generation canvas">
        ...
        >
          <div className="sketch-card rounded-2xl bg-white p-3 shadow-2xl sm:p-4">
            <img src={PINNED.sketch} alt="Sketch of a sneaker" className="h-[min(38vh,280px)] w-auto max-w-full sm:h-[min(46vh,360px)]" />
          </div>
        </div>
      </section>
    )
  ...
  return (
    <section ref={root} className="relative h-[400vh] w-full" aria-label="Phone morphs into AI generation canvas">
      <div
        className="pin-target flex h-screen w-full items-center justify-center overflow-hidden bg-cover bg-center"
        style={{ backgroundImage: `url('${DESK_BG}')` }}
      >
        <div className="phone relative z-10 aspect-[316/587] h-[55vh] max-h-[480px] rounded-[2.5rem] border-[10px] border-black bg-black shadow-2xl sm:h-[62vh] sm:max-h-[540px] md:h-[68vh] md:max-h-[600px]">
          ...
          <div className="absolute inset-0 overflow-hidden rounded-[1.9rem]">
            <div className="screen absolute inset-0"><FeedScreen /></div>
            <div className="screen absolute inset-0"><PromptScreen /></div>
            <div className="screen absolute inset-0"><SlideScreen /></div>
            <div className="screen absolute inset-0 bg-black" aria-hidden />
          </div>
        </div>

        <div
          className="dark-canvas absolute inset-0 z-0 bg-[#111111]"
          style={{
            backgroundImage: 'radial-gradient(rgba(255,255,255,.12) 1px, transparent 1px)',
            backgroundSize: '26px 26px',
          }}
        >
          <div className="sketch-card absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-3 shadow-2xl sm:p-4">
            <img src={PINNED.sketch} alt="Sketch of a sneaker" className="h-[min(38vh,280px)] w-auto max-w-full sm:h-[min(46vh,360px)]" />
          </div>
          <div className="result-thumb thumb-a absolute overflow-hidden rounded-xl">
            <img src={PINNED.result1} alt="Generated sneaker result" className="w-24 sm:w-40" />
          </div>
          <div className="result-thumb thumb-b absolute overflow-hidden rounded-xl">
            <img src={PINNED.result2} alt="Generated sneaker result" className="w-28 sm:w-44" />
          </div>
          <div className="result-thumb thumb-c absolute overflow-hidden rounded-xl">
            <img src={PINNED.result3} alt="Generated sneaker result" className="w-24 sm:w-40" />
          </div>
        </div>
      </div>
    </section>
